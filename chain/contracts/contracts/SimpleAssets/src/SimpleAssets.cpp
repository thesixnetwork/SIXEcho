...
ACTION SimpleAssets::newasset(name author) {
	...
	sassets assets( _self, author.value );
	string empty = "";
	bool no = false;
	...
	//Events
	sendEvent( author, author, "saenewasset"_n, std::make_tuple( author, newID ) );
	SEND_INLINE_ACTION( *this, newassetlog, { {_self, "active"_n} }, { author, newID} );
